...
const TELEGRAM_TOKEN = process.env.TELEGRAM_SUPPORT_BOT_TOKEN;
...
export async function POST(req: Request) {
  ...
  if (bodyText) {
    await Promise.all(OWNER_IDS.map((ownerId) => sendMessage(ownerId, header + bodyText)));
  }

  if (msg.photo || msg.document) {
    await Promise.all(OWNER_IDS.map((ownerId) => forwardMessage(ownerId, chatId, msg.message_id)));
  }

  await sendMessage(chatId, "Спасибо! Ваша заявка отправлена, мы свяжемся с вами при необходимости.");
  return NextResponse.json({ ok: true });
}

export async function GET() {
  return NextResponse.json({ ok: true, message: "Support bot webhook is active" });
}
