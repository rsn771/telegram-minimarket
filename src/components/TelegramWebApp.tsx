...
export function TelegramWebApp() {
  useEffect(() => {
    if (typeof window !== "undefined" && window.Telegram?.WebApp) {
      ...
      try {
        const audio = new Audio("/app-open.mp3");
        audio.volume = 0.9;
        // Игнорируем ошибки авто-воспроизведения (зависят от политики WebView/браузера)
        void audio.play().catch(() => {});
      } catch {
        // Если Audio недоступен — просто ничего не делаем
      }
